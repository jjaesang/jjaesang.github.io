...
> - 브러커는 single thread로 다른 브러커에게 데이터를 복제함, 그래서 하나의 브로커에 파티션이 많을 수록, 복제하는데 시간이 오래걸리고, 그렇게 되면 commit의 delay가 있어 Latency가 증가
> - 해결책, partition 수를 줄이거나, 브러커 서버를 증설해서 브러커 서버당 할당되는 파티션 개수를 줄이자
> - follwer partition이 leader parition을 못 따라가면, 하나씩 증가하면서 테스트해봐야함
...
> - 클러스터의 일시적 오류인해, retry 하게 되면 데이터의 중복이 발생할 수 있음 ( 이런 경우에는, consumer에서 중복을 제거하는 로직이 있어야함)
...
> - producer가 존재하지 않은 topic에 producing 시, auto.create.topic.enable = true 시 토픽이 자동생성
...
> - 브로커 서버가 내려가면, 해당 브로커의 문제를 파악하고, 해당 브로커서버가 가진 leader파티션을 다른 타피션으로 변경함
> - 새로운 leader를 선정하는 방법은, 해당 partition의 ISR 리스트 중 하나를 leader로 선정하고 나머지 follwer들은 새로운 leader parition 정보를 복제 시작
> - unclean.leader.election.enable은 ISR인 아닌 브로커 서버를 leader로 선정할 것 인가에 대한 설정값 
> - 이 값은, commit은 되었지만 복제되지 않은 데이터 유실을 방지해 durablity는 증가시키지만, replica가 sync될 떄 까지 기다리는 시간이 생겨 availablity 측면에서 손해를 봄


5. broker.rack
> - rack 정보를 입력시 , partition replica를 다른 rack으로 분대함

6. log.flush.interval.message/ log.flush.interval.ms
> - kafka는 replication을 통해 durabliity를 높이면서, OS는 page cache로 저장한 데이터를 Disk로 flush할 수 있게 되어 있음
...
- acks = all / min.insync.replicat =2 
