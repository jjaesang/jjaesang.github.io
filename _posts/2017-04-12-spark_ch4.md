...
>각 키에 대한 총합을 계산 하기 전에 각 머신에서 자동으로 병합(combining) 수행  combiner지정할 필요없음
...
이 것이 RDD에서 연산이 처리 될 때 동시 작업의 수준을 결정
 ( 특정 파티션 사용하도록 요청 가능 ) 
default :  클러스터의 사이즈에 맞는 적절한 파티션 개수
RDD의 파티셔닝을 바꾸고 싶다?
repartition() = 네트워크를 통한 데이터교환 ( 셔플링 == 비쌈.. )
 > 최적화 버전 : coalesce() 
개념은 merge과 같음
하나의 합치는 것이 아닌 더 적은 개수로 합친다는 뜻 
  ( = RDD의 파티션 개수를 줄이는 경우에는 셔플링 발생 X )
...
groupByK()
>쌍을 이루지 않거나 key값이 아닌 다른 조건으로 그룹화하는 경우 사용
cogroup()
...
map() 종류의 연산들은 Key값의 변형할 여지가 있기 때문에 ..
어떻게 파티션 되는지 결정해야한다.
...
partitioner은 scala.Option object return ;
 특정 정보를 담기위한 컨테이너 역할
