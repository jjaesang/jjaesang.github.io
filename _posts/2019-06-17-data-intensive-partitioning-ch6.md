...
> - 해당 노드가 병목이 되어 파티셔닝의 목적을 해치기 떄문
...
Hbase, 리싱크DB처럼 키 범위 파티셔닝을 사용하는 데이터베이스는 파티션읋 **동적**으로 만듬
...
- 이 문제를 해결하고자 Hbase나 몽고DB에서는 빈 데이터베이스에 초기 파티션 집합을 서정할 수 있게 함
...
- 무작위로 선책해서 균등하지 않은 분할이 생길 수 있음 
- 하지만, 여러 파티션에 대해 평균적으로 보면 새 노드는 기존 노들이 담당하던 부하에서 균등한 몫을 할당박데 된다
...
모든 경우에 핵심 문제는 **리우팅 결정을 내린는 구성요소가 노드에 할당된 파티션의 변경사항을 어떻게 하는가?**
...
많은 분산 시스템은 **클러스터 메타데이털를 추적하기 위해 Zookeeper같은 별도의 코디네이터 서비스**를 사용
...
- 몽고DB는 자체적인 **설정서버(config server)** 구현에 의존하고 **몽고스(mongos)**데몬을 라우팅 게층에 사용힘
- 카산드라, 리악은 **가십 프로토콜(gossip protocol)** 사용해 클러스터의 상태변활르 노드 사이에 퍼트림
> - 아무나 요청을 받을 수 있고, 요청받은 노드는 적한한 노드에 라우칭함
- 카우치베이스는 재균형화를 자동으로 실행하지 않아, 클러스터 노드로부터 변경된 라우팅 정보를 알아내는 목시(moxi)라는 라윙 계층을 설정

클라이언트는 라우팅 계층을 사용하든, 노드에 요청을 보낼 떄도 접속할 IP를 주소를 알아야함
